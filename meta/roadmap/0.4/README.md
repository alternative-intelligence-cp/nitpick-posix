# Cycle 0.4 — Text streams II

`sort`, `uniq`, `comm`, `join`, `split`, `csplit`, `od`, `cmp`, `strings`, `cksum`, `tsort`.
...
`sort` agreeing with the reference over a generated corpus of key specifications, and every numeric argument surviving the S-9 corpus.

## Watch for

- **`sort`'s key syntax is a small language of its own and is where the numeric-argument hazard is exercised hardest.
