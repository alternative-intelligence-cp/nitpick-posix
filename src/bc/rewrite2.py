...
# 1. Replace ALL `flt64` with `fix256` generally (except in the wrappers we add later)
content = content.replace("flt64", "fix256")

# Revert specific things that might have been broken
...
content = content.replace('fmod(left, right)', 'raw(left % right)')

content = content.replace('lexer_curr_val = atof(num_str);', 'lexer_curr_val = raw(str_to_fix256(num_str));')

# Fix division returning Result
content = content.replace('left = left / right;', 'left = raw(left / right);')
content = content.replace('left = left * right;', 'left = raw(left * right);')
content = content.replace('left = left + right;', 'left = raw(left + right);')
content = content.replace('left = left - right;', 'left = raw(left - right);')
