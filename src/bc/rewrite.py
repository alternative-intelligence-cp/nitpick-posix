...
# 1. Replace all flt64 with fix256 where applicable
content = re.sub(r'flt64:([a-zA-Z0-9_]+)', r'fix256:\1', content)
content = re.sub(r'= 0\.0f64', r'= @cast_unchecked<fix256>(0.0)', content)
content = re.sub(r'pass\(0\.0f64\)', r'pass(@cast_unchecked<fix256>(0.0))', content)
content = re.sub(r'!= 0\.0f64', r'!= @cast_unchecked<fix256>(0.0)', content)
content = content.replace('0.0f64 - val', '@cast_unchecked<fix256>(0.0) - val')
content = content.replace('flt64()', 'fix256()')

# 2. Remove extern libc functions we don't want
content = re.sub(r'extern func:pow = flt64\(flt64:x, flt64:y\);\n', '', content)
content = re.sub(r'extern func:fmod = flt64\(flt64:x, flt64:y\);\n', '', content)
content = re.sub(r'extern func:sin = flt64\(flt64:x\);\n', '', content)
content = re.sub(r'extern func:cos = flt64\(flt64:x\);\n', '', content)
content = re.sub(r'extern func:tan = flt64\(flt64:x\);\n', '', content)
content = re.sub(r'extern func:sqrt = flt64\(flt64:x\);\n', '', content)
content = re.sub(r'extern func:exp = flt64\(flt64:x\);\n', '', content)
content = re.sub(r'extern func:log = flt64\(flt64:x\);\n', '', content)
content = re.sub(r'extern func:strcmp = int64\(string:s1, string:s2\);\n', '', content)
content = re.sub(r'extern func:atof = flt64\(string:str\);\n', '', content)
...
# Insert wrappers after lexer variables
...
content = content.replace('sin(val)', 'bc_sin(val)')
content = content.replace('cos(val)', 'bc_cos(val)')
content = content.replace('tan(val)', 'bc_tan(val)')
content = content.replace('sqrt(val)', 'bc_sqrt(val)')
content = content.replace('exp(val)', 'bc_exp(val)')
content = content.replace('log(val)', 'bc_log(val)')
content = content.replace('pow(left, right)', 'bc_pow(left, right)')
...
content = re.sub(r'extern func:sprintf = int32\(any\*:buf, string:fmt, flt64:val\);\n', '', content)
...
old_print_logic = """            any->:out_buf = @cast_unchecked<int64>(alloc(256i64)) => any->;
...
content = content.replace('int64:bytes_read = read(0i32, buf, 4095i64);', 'int64:bytes_read = sys!!(0i64, 0i64, buf, 4095i64);')
